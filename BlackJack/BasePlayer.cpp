...
//hand sum.
void BasePlayer::insertCardToHand(std::vector <unsigned int> newCard, unsigned int whichHand)
{
...
void BasePlayer::printHand(unsigned int whichHand, unsigned int aiOrNot) const
{
...
unsigned int BasePlayer::returnSum() const
{ 
	return handOneSum; 
}

//Getter; returns a card
unsigned int BasePlayer::returnThatCard(unsigned int i, unsigned int whichHand) const
{
	if (0 == whichHand)
	{
		return handOne[i][2];
